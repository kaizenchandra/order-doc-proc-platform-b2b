...
class MessagingIT {
    ...
    @BeforeAll static void start() {
        ...
        try {
            ...
            orders = context.getBean(OrderRepository.class); documents = context.getBean(OrderDocumentRepository.class);
            journal = context.getBean(OrderJournal.class); outbox = context.getBean(OutboxStore.class);
            codec = context.getBean(EventCodec.class); handler = context.getBean(DocumentResultHandler.class);
            receiver = context.getBean(ResultReceiver.class); settings = context.getBean(MessagingProperties.class);
        } catch (RuntimeException | Error error) { stop(); throw error; }
    }
    @AfterAll static void stop() {
        try { if (context != null) context.close(); } finally { if (database != null) database.close(); }
    }
    @BeforeEach void clear() { jdbc.execute("truncate orders, order_documents, outbox_events, inbox_events, api_idempotency"); }
    private OutboxEvent pending() {
        ...
        var saved = event; tx.executeWithoutResult(s -> journal.append(saved)); return event;
    }
    private OutboxRelay relay(EventPublisher publisher) { return new OutboxRelay(outbox, codec, publisher, settings); }
    private void expire(UUID id) { jdbc.update("update outbox_events set lease_until = now() - interval '1 second' where event_id = ?", id); }
    private OrderDocument queued() {
        ...
        tx.executeWithoutResult(s -> { orders.save(order); documents.save(doc); }); return doc;
    }
    ...
    private OrderDocument reload(OrderDocument doc) { return documents.findByTenantIdAndOrderIdAndId(doc.tenantId(), doc.orderId(), doc.id()).orElseThrow(); }
    private int inboxCount() { return jdbc.queryForObject("select count(*) from inbox_events", Integer.class); }

    @Test void relayPublishesOutsideTransactionAndMarksOnlyAcceptedMessages() {
        var event = pending(); var calls = new AtomicInteger();
        assertEquals(1, relay((destination, bytes, attributes) -> {
            assertFalse(TransactionSynchronizationManager.isActualTransactionActive());
            assertEquals("order-events", destination); assertEquals(event.eventId(), codec.decode(bytes).eventId());
    ...
    @Test void failureBacksOffAndRetainsEventForRetry() {
        var event = pending();
        relay((destination, bytes, attributes) -> { throw new IllegalStateException("broker unavailable"); }).poll();
        ...
        var lease = outbox.claim().orElseThrow(); assertEquals(2, lease.attempt()); assertEquals(event.eventId(), lease.eventId());
    }
    @Test void expiredLeaseCannotMarkOrRescheduleAnotherWorkersEvent() {
        pending(); var old = outbox.claim().orElseThrow(); expire(old.eventId()); var replacement = outbox.claim().orElseThrow();
        assertNotEquals(old.token(), replacement.token());
        assertFalse(outbox.published(old)); assertFalse(outbox.retry(old, 1, "PUBLISH_FAILED"));
        assertTrue(outbox.published(replacement));
    }
    @Test void concurrentWorkersClaimDisjointRows() throws Exception {
        for (int i = 0; i < 4; i++) pending();
        var gate = new CountDownLatch(1);
        try (var executor = Executors.newFixedThreadPool(4)) {
            var futures = new ArrayList<Future<UUID>>();
            for (int i = 0; i < 4; i++) futures.add(executor.submit(() -> { assertTrue(gate.await(5, TimeUnit.SECONDS)); return outbox.claim().orElseThrow().eventId(); }));
            gate.countDown(); var ids = new HashSet<UUID>();
            for (var future : futures) ids.add(future.get(10, TimeUnit.SECONDS));
            assertEquals(4, ids.size()); assertTrue(outbox.claim().isEmpty());
        }
    }
    @Test void crashAfterPublishReusesTheSameEnvelope() {
        pending(); var abandoned = outbox.claim().orElseThrow(); byte[] first = codec.encode(abandoned.envelope(codec));
    ...
    @Test void invalidOutboxPayloadIsRetainedWithoutCallingPublisher() {
        var event = pending(); jdbc.update("update outbox_events set event_version = 2 where event_id = ?", event.eventId());
    ...
    @Test void ackOccursAfterCommitAndLostAckRedeliveryDoesNotReapply() {
        var doc = queued(); var event = result(doc, doc.processingRequestId(), true);
        assertThrows(IllegalStateException.class, () -> receiver.receive(codec.encode(event), new ResultReceiver.Acknowledgement() {
            public void ack() {
                assertFalse(TransactionSynchronizationManager.isActualTransactionActive());
                assertEquals(DocumentStatus.PROCESSED, reload(doc).status()); assertEquals(1, inboxCount());
                throw new IllegalStateException("connection lost during ACK");
            }
            public void nack() { fail("Must not NACK a committed result"); }
        }));
        assertEquals(DocumentResultHandler.Outcome.DUPLICATE, handler.handle(event));
        assertEquals(1L, reload(doc).version()); assertEquals(1, inboxCount());
    }
    @Test void databaseFailureNacksAndRollsBackInboxAndDocument() {
        var doc = queued(); var event = result(doc, doc.processingRequestId(), true); var nacks = new AtomicInteger();
        ...
        try {
            receiver.receive(codec.encode(event), new ResultReceiver.Acknowledgement() {
                public void ack() { fail("Must not ACK a failed transaction"); }
                public void nack() { nacks.incrementAndGet(); }
            });
            assertEquals(1, nacks.get()); assertEquals(0, inboxCount()); assertEquals(DocumentStatus.QUEUED, reload(doc).status());
        } finally { jdbc.execute("alter table order_documents drop constraint test_reject_result"); }
        assertEquals(DocumentResultHandler.Outcome.APPLIED, handler.handle(event)); assertEquals(1, inboxCount());
    }
    @Test void concurrentDuplicateResultsCommitOneEffect() throws Exception {
        var doc = queued(); var event = result(doc, doc.processingRequestId(), true); var gate = new CountDownLatch(1);
        try (var executor = Executors.newFixedThreadPool(3)) {
            var futures = new ArrayList<Future<DocumentResultHandler.Outcome>>();
            for (int i = 0; i < 3; i++) futures.add(executor.submit(() -> { gate.await(); return handler.handle(event); }));
            gate.countDown(); int applied = 0;
            for (var future : futures) if (future.get(10, TimeUnit.SECONDS) == DocumentResultHandler.Outcome.APPLIED) applied++;
            assertEquals(1, applied); assertEquals(1, inboxCount()); assertEquals(1L, reload(doc).version());
        }
    }
    @Test void staleResultCannotOverwriteReprocessingOrTerminalState() {
        var doc = queued(); assertEquals(DocumentResultHandler.Outcome.APPLIED, handler.handle(result(doc, doc.processingRequestId(), false)));
        ...
        assertEquals(DocumentStatus.PROCESSED, reload(doc).status()); assertEquals(4, inboxCount());
    }
    @Test void untrustedReportAndWrongTenantAreRejectedWithoutInboxCommit() {
        var doc = queued(); var good = result(doc, doc.processingRequestId(), true); var data = (Events.DocumentResult) good.data();
        ...
        assertThrows(ApiFailure.class, () -> handler.handle(wrongTenant)); assertEquals(0, inboxCount());
        assertEquals(DocumentStatus.QUEUED, reload(doc).status());
    }
}
