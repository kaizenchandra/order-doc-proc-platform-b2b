...
class EventCodecTest {
    ...
    @Test void roundTripIsTypedAndAcceptsAdditiveFields() {
        var event = event();
    ...
    @Test void malformedDuplicateOversizedAndUnsupportedEventsFailClosed() {
        var wire = new String(codec.encode(event()), StandardCharsets.UTF_8);
    ...
    @Test void traceContextRejectsZeroIdsAndRestoresThreadState() {
        ...
        try (var ignored = TraceContext.open(trace)) {
            ...
            try (var nested = TraceContext.open("bad")) { assertNull(TraceContext.current()); }
            assertEquals(trace, TraceContext.current());
        }
