...
import com.synechisveltiosi.platform.order.application.DocumentWorkflows;
import com.synechisveltiosi.platform.order.application.OrderWorkflows;
...
public class OrderController {
    ...
    public OrderController(OrderWorkflows orders, DocumentWorkflows documents) {
        this.orders = orders;
    ...
    @GetMapping("/{orderId}")
    public OrderView get(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID orderId) {
        return orders.get(tenant(jwt), orderId);
    }
    ...
    private UUID correlation(HttpServletRequest request) {
        return (UUID) request.getAttribute(CorrelationFilter.ATTRIBUTE);
    }
}
