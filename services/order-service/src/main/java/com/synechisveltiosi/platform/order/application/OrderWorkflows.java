...
import com.synechisveltiosi.platform.order.adapter.persistence.OutboxEvent;
...
public class OrderWorkflows {
    ...
    public OrderView changeStatus(UUID tenant, UUID orderId, ChangeStatus input, UUID correlation) {
        ...
        if (!order.version().equals(input.expectedVersion()))
            throw ApiFailure.conflict("Order version has changed; read the order again");
    ...
    public DocumentView complete(UUID tenant, UUID orderId, UUID documentId, VerifiedUpload upload, UUID correlation) {
        ...
        requireOpen(order);
        try {
            document.queue(upload, UUID.randomUUID(), "1", clock.instant());
