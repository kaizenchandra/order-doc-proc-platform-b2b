...
public class OutboxEvent {
    ...
    @Column(nullable = false, updatable = false) private UUID aggregateId;
    @Column(nullable = false, updatable = false, length = 100) private String eventType;
    @Column(nullable = false, updatable = false) private int eventVersion;
    @Column(nullable = false, updatable = false) private UUID correlationId;
    @Column(updatable = false) private UUID causationId;
    @Column(nullable = false, updatable = false) private Instant occurredAt;
    @Column(nullable = false, updatable = false, length = 100) private String source;
    @Column(nullable = false, updatable = false, length = 100) private String destination;
    @JdbcTypeCode(SqlTypes.JSON) @Column(nullable = false, updatable = false, columnDefinition = "jsonb") private String data;
    @Column(updatable = false, length = 128) private String traceparent;
