...
public class ApiErrors extends ResponseEntityExceptionHandler {
    ...
    ProblemDetail application(ApiFailure error) {
    ...
    ProblemDetail invalid(RuntimeException error) {
        return ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, "Request values are invalid");
    }

    @ExceptionHandler({OptimisticLockingFailureException.class, DataIntegrityViolationException.class})
    ProblemDetail conflict(RuntimeException error) {
        return ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, "The request conflicts with current state");
    }

    @ExceptionHandler({TransientDataAccessException.class, DataAccessResourceFailureException.class})
    ProblemDetail unavailable(RuntimeException error) {
        return ProblemDetail.forStatusAndDetail(HttpStatus.SERVICE_UNAVAILABLE, "Persistence is temporarily unavailable; retry the request");
    }
