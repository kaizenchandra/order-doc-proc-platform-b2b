...
public class OrderDocument {
    ...
    @Column(nullable = false, updatable = false) private UUID orderId;
    @Column(nullable = false, length = 255, updatable = false) private String fileName;
    @Column(nullable = false, length = 100, updatable = false) private String declaredContentType;
    @Column(nullable = false, length = 222, updatable = false) private String bucket;
    @Column(nullable = false, length = 512, updatable = false) private String objectName;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 24) private DocumentStatus status;
    @Column(nullable = false, updatable = false) private Instant uploadExpiresAt;
    private Long objectGeneration;
    private Long sizeBytes;
    private UUID processingRequestId;
    @Column(length = 32) private String processorVersion;
    private Instant queuedAt;
    private Instant completedAt;
    @Column(length = 222) private String reportBucket;
    @Column(length = 512) private String reportObjectName;
    ...
    public static OrderDocument register(UUID id, UUID tenantId, UUID orderId, String fileName,
        ...
        if (!Objects.requireNonNull(expiresAt).isAfter(now)) throw new IllegalArgumentException("Upload window must be positive");
    ...
    public void reprocess(UUID newRequestId, String processor, Instant now) {
        ...
        if (newRequestId.equals(processingRequestId)) throw new IllegalArgumentException("Reprocessing requires a new request ID");
        DomainChecks.text(processor, 32, "processorVersion");
        DomainChecks.time(now, updatedAt);
    ...
    public boolean succeed(UUID requestId, GcsObjectReference report, String checksum, Instant now) {
        ...
        if (checksum == null || !checksum.matches("[0-9a-f]{64}")) throw new IllegalArgumentException("Invalid SHA-256");
        DomainChecks.time(now, updatedAt);
    ...
    public Long objectGeneration() { return objectGeneration; }
    public Long sizeBytes() { return sizeBytes; }
    public UUID processingRequestId() { return processingRequestId; }
    public String processorVersion() { return processorVersion; }
    public Instant queuedAt() { return queuedAt; }
    public Instant completedAt() { return completedAt; }
    public String reportBucket() { return reportBucket; }
    public String reportObjectName() { return reportObjectName; }
    public Long reportGeneration() { return reportGeneration; }
