...
public class OrderJournal {
    ...
    public OrderJournal(EntityManager entityManager) { this.entityManager = entityManager; }

    // persist (not merge) prevents a reused event ID from rewriting a committed event.
    @Transactional(propagation = Propagation.MANDATORY)
    public void append(OutboxEvent event) { entityManager.persist(event); }

    @Transactional(propagation = Propagation.MANDATORY)
    public void remember(IdempotencyRecord record) { entityManager.persist(record); }
