...
public final class RequestEventCodec {
    ...
    public Events.Envelope decode(byte[] data) {
        ...
        if (!root.path("eventVersion").isIntegralNumber() || !root.path("eventVersion").canConvertToInt()
                ...
                || !"DocumentProcessingRequested".equals(text(root, "eventType")))
            throw new IllegalArgumentException("Unsupported event");
        var payload = root.path("data");
        var request = new Events.DocumentProcessingRequested(uuid(payload, "documentId"), uuid(payload, "processingRequestId"),
                text(payload, "bucket"), text(payload, "objectName"), text(payload, "generation"), text(payload, "processorVersion"));
        return new Events.Envelope(uuid(root, "eventId"), "DocumentProcessingRequested", 1, uuid(root, "tenantId"),
