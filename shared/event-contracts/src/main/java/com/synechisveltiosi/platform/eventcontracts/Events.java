...
public final class Events {
     ...
     */
    public static String reportObject(UUID tenant, UUID request) {
        return tenant + "/" + request + "/result.json";
    }
    ...
    public record Envelope(UUID eventId, String eventType, int eventVersion, UUID tenantId, UUID aggregateId,
        ...
        public Envelope {
            ...
            boolean valid = switch (eventType) {
                case "OrderCreated" ->
                        data instanceof OrderCreated d && d.orderId().equals(aggregateId) && "order-service".equals(source);
                case "OrderStatusChanged" ->
                        data instanceof OrderStatusChanged d && d.orderId().equals(aggregateId) && "order-service".equals(source);
                case "DocumentProcessingRequested" ->
                        data instanceof DocumentProcessingRequested && "order-service".equals(source);
                case "DocumentProcessed" ->
                        data instanceof DocumentResult d && d.sha256() != null && d.failureCode() == null && "document-service".equals(source);
    ...
    public record OrderCreated(UUID orderId, UUID customerId, String status, BigDecimal totalAmount,
                               String currency) implements Data {
        public OrderCreated {
    ...
    public record DocumentProcessingRequested(UUID documentId, UUID processingRequestId, String bucket,
                                              String objectName, String generation,
                                              String processorVersion) implements Data {
        public DocumentProcessingRequested {
            Objects.requireNonNull(documentId);
            Objects.requireNonNull(processingRequestId);
            text(bucket, 222);
            text(objectName, 512);
            Events.generation(generation);
            text(processorVersion, 32);
        }
    }

    public record DocumentResult(UUID documentId, UUID processingRequestId, String processorVersion,
                                 String reportBucket, String reportObjectName, String reportGeneration, String sha256,
                                 String failureCode) implements Data {
        public DocumentResult {
            Objects.requireNonNull(documentId);
            Objects.requireNonNull(processingRequestId);
            text(processorVersion, 32);
            text(reportBucket, 222);
            text(reportObjectName, 512);
            Events.generation(reportGeneration);
            if ((sha256 == null) == (failureCode == null))
                throw new IllegalArgumentException("Expected one result outcome");
